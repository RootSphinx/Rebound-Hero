...
public class Transformation {
    ...
    public static Matrix4f getWorldMatrix(GameObject gameObject,Camera camera){

        return matrix.identity()
                .ortho2D(camera.getPosition().x - camera.getWidth()/2f,
                        camera.getPosition().x + camera.getWidth()/2f,
                        camera.getPosition().y - camera.getHeight()/2f,
                        camera.getPosition().y + camera.getHeight()/2f)
